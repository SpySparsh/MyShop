...
export default function AllUsers() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get('https://myshop-4-rjis.onrender.com/api/users', {
        headers: {
  ...
  const deleteUser = async (id) => {
    ...
    try {
      await axios.delete(`https://myshop-4-rjis.onrender.com/api/users/${id}`, {
  ...
  const promoteUser = async (id) => {
    try {
      await axios.put(`https://myshop-4-rjis.onrender.com/api/users/${id}/promote`, {}, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      fetchUsers();
    } catch (err) {
